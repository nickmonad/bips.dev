...
-   If NODE\_NETWORK is specified, then \"getdata\" for transactions
    will retrieve them not only from the memory pool but also check the
...
the Bitcoin network (see relaying), but cannot support \"getblocks\" as
it does not have the entire blockchain.
