...
`musig()` expressions take multiple keys and produce an aggregate public key using <a href="/327" target="_blank">BIP-327</a>.
...
In the following sections, the term `KEY` refers to key expressions as defined in <a href="/380" target="_blank">BIP-380</a>
...
with the `KeySort` algorithm after all derivation and prior to aggregation<ref>**Why must
...
`musig(KEY, KEY, ..., KEY)/NUM/.../*` expressions are also allowed if no KEY expression
contains child derivation as specified by `/*` or multipath as specified with
`/<NUM;NUM;...>`, in addition to the same usage restrictions as in the previous section. The KEY expressions
...
<a href="/328" target="_blank">BIP-328</a>. The `/NUM/.../*` following the `musig()` specifies
the derivation path to be used when deriving from the aggregate public key. `/*` is also
optional. As there is no aggregate private key, only unhardened derivation from the aggregate public
key is allowed. Thus these derivation steps cannot contain `/NUMh` or `/NUM'`
nor can child derivation be specified as `/*h`, or `/*'`.
