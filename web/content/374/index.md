...
*  Let _R<sub>1</sub> = k⋅G_.
*  Let _R<sub>2</sub> = k⋅B_.
*  Let _e = int(hash<sub>BIP0374/challenge</sub>(cbytes(A) || cbytes(B) || cbytes(C) || cbytes(G) || cbytes(R<sub>1</sub>) || cbytes(R<sub>2</sub>) || m')) mod n_.
*  Let _s = (k + e⋅a) mod n_.
...
*  Let _e = int(proof[0:32])_; fail if _e &ge; n_.
...
*  Let _m' = m if m is provided, otherwise an empty byte array_.
*  Fail if _e ≠ int(hash<sub>BIP0374/challenge</sub>(cbytes(A) || cbytes(B) || cbytes(C) || cbytes(G) || cbytes(R<sub>1</sub>) || cbytes(R<sub>2</sub>) || m')) mod n_.
...
*  0.3.0 (2026-08-19):
    *  Treat the challenge _e_ as a scalar (reduced modulo the curve order) and reject proofs with _e &ge; n_
*  0.2.0 (2025-02-27):
