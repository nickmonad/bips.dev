...
1.  Validate the merchant\'s identity and signature using the PKI
    system, if the pki\_type is not \"none\".
...
PaymentRequest with merchant\_data set to \"lang=el&basketId=11252\".
The customer pays, their bitcoin client sends a Payment message, and the
...
If pki\_type is \"x509+sha256\", then the PaymentRequest message is
hashed using the SHA256 algorithm to produce the message digest that is
signed. If pki\_type is \"x509+sha1\", then the SHA1 algorithm is used.
