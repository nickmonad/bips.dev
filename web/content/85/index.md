...
_One Seed to rule them all,_<br>
...
Derivation path uses the format `m/83696968'/{app}'/{index}'` where _{app}_ is the **path** for the application, and _{index}_ is the index.
...
private key<ref name="curve-order" />. Prepend an empty byte (`0x00`)
