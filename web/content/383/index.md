...
The values `k` and `n` must be minimally encoded integers.
For values less than or equal to 16, they must be encoded using `OP_0` through `OP_16`.
...
`multi()`, and `sortedmulti()` descriptors use the format and general operation specified in <a href="/380" target="_blank">380</a>.
