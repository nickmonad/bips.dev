...
outputs, the PSBT\_GLOBAL\_TX\_MODIFIABLE field must be present and the
Inputs Modifiable and Outputs Modifiable flags set appropriately;
moreover, the transaction version number must be set to at least 2.
\[3\] If the Creator is a Constructor and no inputs and outputs will be
added by other entities, PSBT\_GLOBAL\_TX\_MODIFIABLE may be omitted.
...
<!-- end list -->
  ...
  - Case: PSBTv0 but with PSBT\_GLOBAL\_TX\_MODIFIABLE.
      - Bytes in Hex:
