...
eavesdropper.
  ...
  - Encryption, even when it is unauthenticated and only used when both
    ...
    intensive in general, but in the case of manual, deliberate
    connections (as opposed to automatic, random ones), they are also in
    principle detectable: even very basic checks, e.g., operators
    manually comparing protocol versions and session IDs (as supported
    by the proposed protocol), will expose the attacker.
...
context of secure channel protocols is ambiguous. It can refer to:
  ...
  - The communication protocol establishing that the communication
    ...
    than no encryption. Thus, all connections should use encryption,
    even if they are unauthenticated.
...
node operators instruct their software to make, and the automatic
connections Bitcoin nodes make with each other based on IP addresses
obtained via gossip. While encryption per se is already possible with
proxy networks or VPN protocols, these are not desirable or applicable
for automatic connections at scale:
  ...
  - VPN protocols like WireGuard or OpenVPN inherently define a private
...
certificates) is at the core of the design of existing general-purpose
...
In contrast, our proposal instead aims for a simple modular design that
...
authentication alongside the Diffie-Hellman key exchange.\[2\] However,
the resulting increase in connection establishment latency is a not a
concern for Bitcoin's long-lived connections, [which typically live for
hours or even weeks](https://www.dsn.kastel.kit.edu/bitcoin/).
...
\#\*\* Send their 16-byte garbage terminator.\[13\]
...
long as progress is guaranteed.<ref name="handshake_progress">**How can
progress be guaranteed in a backwards-compatible way?** In order to
guarantee progress, it must be ensured that no deadlock occurs, i.e., no
state is reached in which each party waits for the other party
indefinitely. For example, any upgrade that adheres to the following
conditions will guarantee progress:
  ...
  - The responder must start sending after having received at least one
    byte that mismatches that 16-byte prefix.
  ...
  - After either party has sent their garbage terminator, they must
    transition to the version negotiation phase without waiting for more
    bytes.
...
jointly called **the handshake**, comprise just 1.5 roundtrips:
  ...
  - the responder sends public key + garbage + garbage terminator +
    decoy packets (optional) + version packet
...
Each packet consists of:
  ...
  - An authenticated encryption of the **plaintext**, which consists of:
      - A 1-byte **header** which consists of transport layer protocol
        flags. Currently, only the highest bit is defined as the
        **ignore bit**. The other bits are ignored, but this may change
        in future versions\[18\].
...
are:
  ...
  - A hash step is performed every 224\[21\] messages to rekey the
...
Packet encryption is built on two existing primitives:
  ...
  - The **ChaCha20 Block Function** is specified in [RFC 8439
    section 2.3](https://datatracker.ietf.org/doc/html/rfc8439#section-2.3).
...
rekeys every 224 messages by encrypting 32 zero bytes\[32\], and using
...
than once per direction per connection.\[33\]\<ref
name"fixed\_length\_long\_ids"\>**Why not allow variable length long
...
message types to fixed lengths of 1 or 13 hampers traffic
analysis.</ref>
...
algorithms. This code is for demonstration purposes only:
  ...
  - [XElligatorSwift decoding
    vectors](bip-0324/ellswift_decode_test_vectors.csv "wikilink")
    provide examples of ElligatorSwift-encoded public keys, and the X
    coordinate they map to.
  - [XSwiftECInv
...
8.  **What if a v2 initiator's public key starts accidentally with these
    16 bytes?** This is so unlikely (probability of *2<sup>-128</sup>*)
    to happen randomly in the v2 protocol that the initiator does not
    need to specifically avoid it. The optional detection of
    wrong-network v1 peers has a probability of *2<sup>-96</sup>*, which
    is still negligible compared to random network failures.
...
13. **Why does the protocol need a garbage terminator?** While it is in
    principle possible to use the first packet after the garbage
    ...
    received byte.
14. **Why does the protocol authenticate the garbage?** Without garbage
    authentication, the garbage would be modifiable by a third party
    without consequences. We want to force any active attacker to have
    to maintain a full protocol state. In addition, such malleability
...
18. **Why is the header a part of the plaintext and not included
    ...
    message.
19. **Why is ChaCha20Poly1305 chosen as the basis for packet
...
31. **Why use HKDF for deriving key material?** The shared secret
    ...
    setup.
32. **Why is rekeying implemented in terms of an invocation of the
...
33. **How do the lengths between v1 and v2 compare?** For messages that
    use the 1-byte short message type ID, v2 packets use 3 bytes less
    per message than v1.
34. **Why are v2 clients met with immediate disconnection encouraged to
